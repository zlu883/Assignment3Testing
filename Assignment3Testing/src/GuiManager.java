...
import AvConvObjects.AvConvObject;
import AvConvObjects.SaveFileManager;
import Dialogs.*;
import Misc.*;
...
public class GuiManager {
	...
	private JMenu _fileMenu, _mediaMenu, _audioMenu, _effectsMenu;
	private JMenuItem _openProject, _saveProject, _exportProject, _playMedia, _downloadMedia, _extractAudio,_overlayAudio, _replaceAudio, 
	...
	private SaveFileManager _sfManager = new SaveFileManager();
	...
	private ActionListener setExtractAudioButton() {
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				new Dialogs.ExtractAudioDialog(_vamixWindow, _currentPlayingFile);
	...
	private ActionListener setExportButton() {
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Dialogs.ExportWindow.getInstance().showGUI();
			}
			
		};
	}
	
	public ActionListener setOpenProjectButton() {
		return new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				String openPath = promptFileChooser(OPEN, "Open Project", _vamixWindow);
				ArrayList<AvConvObject> acObjectList = ListOfAvConvObjects.getWholeList();
				try {
					acObjectList = _sfManager.readSaveFile(openPath);
				} catch (VamixException | IOException e1) {}				
			}
			
		};
	}
	
	private ActionListener setSaveProjectButton() {
		return new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				String savePath = promptFileChooser(SAVE, "Save Project", _vamixWindow);
				ArrayList<AvConvObject> acObjectList = ListOfAvConvObjects.getWholeList();
				try {
					_sfManager.createSaveFile(acObjectList, savePath);
				} catch (VamixException | IOException e1) {}	
